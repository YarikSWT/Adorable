...
- [x] Финальный e2e (2026-04-23, iter 19): Z.ai биллинг unblocked (пополнение). Playwright MCP полный прогон: home → промпт "Write a minimal Express server… Hello from GLM" → POST /api/repos 200 (Gitea repo + Docker sandbox), POST /api/chat 200 в 108s (streaming GLM-5.1 tool use через `lib/adapters/llm.ts`), agent: writeFile server.js, npm install express, start server, verify with node http.get → "Hello from GLM". Второй прогон поставил `index.html` + node http.createServer на :3000 → preview через Caddy `http://<sandboxId>.preview.localhost:8080/` отдал валидный HTML (200). Скриншоты: `verification/screenshots/final-e2e-prod.png` (preview HTML), `final-e2e-home.png`, `final-e2e-chat.png`. Попутный фикс iter 19: sandboxId >63 байт ломал docker DNS (RFC 1035) → добавлен truncate + hash в `sandbox-docker.ts`.
