...
- [x] Список токенов: таблица (имя, kind, prefix, created, lastUsed, expires, статус), кнопка «Отозвать».
- [x] «Создать токен» → модалка с полями name/kind/expiration → submit → закрытие старой модалки и **немедленное открытие** второй модалки с **plaintext-токеном** (моноширно), кнопкой copy и предупреждением «единственный раз». Закрытие — только через «Понятно».
- [x] Отзыв — с подтверждением.
...
- [x] Если не опубликован — CTA-блок с кнопкой «Опубликовать» → диалог выбора visibility (radio с пояснениями) → POST `/api/repos/:id/promote`.
- [x] Если опубликован — URL поддомена + copy, текущий visibility-pill + «Изменить», метаданные (когда, какой commit), кнопка «Опубликовать снова», информер.
- [x] Раздел «Кастомный домен» **скрыт**.
...
- [x] Billing страница (Документ 3, 8.1): карточка плана, таблица лимитов с прогресс-барами, override-блок (если есть), история событий (последние 50).
- [x] Inline-баннер: на home если квота >80% — компактный баннер с CTA-ссылкой на billing. Dismiss с возвратом через 24 часа.
...
- [x] `/admin` — read-only дашборд: счётчики юзеров/orgs/публикаций.
- [x] `/admin/users` — список с поиском + клик на строку.
- [x] `/admin/users/<id>` — карточка + кнопки suspend/unsuspend/смена email.
- [x] `/admin/orgs/<id>` — план, лимиты, override-форма.
- [x] `/admin/audit` — таблица с фильтрами.
