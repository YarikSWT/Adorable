...
export default function ArchiveMessageSettingsPage() {
  ...
  const { data: template, isLoading } = useQuery({
    queryKey: ['archiveMessageTemplate'],
    queryFn: async () => {
      try {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (templateId) {
        ...
        return await base44.entities.ArchiveMessageTemplate.update(templateId, data);
      } else {
        // Создаем новый шаблон
        return await base44.entities.ArchiveMessageTemplate.create({
          ...data,
          is_active: true
        });
      }
    ...
    onSuccess: (savedTemplate) => {
      queryClient.invalidateQueries({ queryKey: ['archiveMessageTemplate'] });
      setTemplateId(savedTemplate.id);
      toast({
        title: "Шаблон сохранен",
        description: "Приветственное сообщение успешно обновлено",
      });
    },
    onError: (error) => {
      toast({
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-600">
            Редактируйте шаблон сообщения, которое будет копироваться вместе со ссылкой на архив
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <Card className="border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                Используйте плейсхолдеры для подстановки данных ребенка и ссылки на архив
              </CardDescription>
            </CardHeader>
